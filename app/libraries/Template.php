...
class Template
{
    public $javascript_lib = '';
    public $css_lib = '';
    /**
     * define bootstrap columns
     *
     * @var array
     */
    public $col_bt = array(
    ...
    public $form_elements = array(
        "form-control" => _FORM_CONTROL, "btn btn_success btn-block" => _SAVE_BTN
    );

    /**
     * load bootstrap admin panel
     *
     * @param string $path
     * @param string $data
     ...
     * @return void
     */
    public function load($path, $data = null, $dataHeader = null, $dataFooter = null)
    {
        $CI = &get_instance();
        if (!$CI->system->checkLogin()) redirect("login");
        ...
        $this->load_pnotify();
        //load css and js
        $dataHeader['bsjas'] = $this->javascript_lib;
        $dataHeader['bscss'] = $this->css_lib;

        if (@$CI->input->post("noheader")) {
            $CI->load->view("layout/header2", $dataHeader);
            $CI->load->view($path, $data);
            $CI->load->view("layout/footer", $dataFooter);
        } else {
            $CI->load->view("layout/header", $dataHeader);
            $CI->load->view($path, $data);
            $CI->load->view("layout/footer", $dataFooter);
        }
    }
    ...
    public function load_min_bt()
    {
        ...
        $this->load_bootstrapjs();
        $this->load_bootstrapcss();
        return $this->javascript_lib . $this->css_lib;
    }
    ...
    /**
     * load without menu for login page 
     *
     * @param string $path
     * @param string $data
     * @param string $dataHeader
     ...
     * @return void
     */
    public function loadLogin($path, $data = null, $dataHeader = null, $dataFooter = null)
    {
        $CI = &get_instance();
        ...
        $bsjas = $this->javascript_lib;
        $bscss = $this->css_lib;
        $dataHeader = array('bsjas' => $bsjas, 'bscss' => $bscss, "title" => _LOGIN . "-" . $CI->system->get_setting('login_title') . "-CANDO");
        $CI->load->view("layout/header2", $dataHeader);
        $CI->load->view($path, $data);
    }
    /**
    ...
    public function load_popup($view, $title = "", $data = array(), $return = false)
    {
        ...
        $footer = '';
        $CI = &get_instance();
        $body = $header . $CI->load->view($view, $data, true) . $footer;
    ...
    /**
     * standard load js  automatic load javascript in header tag
     */
    public function load_js($name, $folder = "assets/js", $base = "")
    {
        $this->javascript_lib .= "<script type='text/javascript' src='" . base_url() . "$base/$folder/$name.js'" . "> </script> \n";
    }
    ...
    /**
     * load select2 library javascript for replace drop down form element
     */
    ...
    /**
     * default bootstrap javascript library
     */

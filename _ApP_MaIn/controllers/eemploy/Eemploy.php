...
class Eemploy extends CI_Controller
{
    ...
    function edit()
    {
        ...
        $ret=$this->eemploy_model->edit_user($this->input->post("id", true));
        $this->template->load_popup('eemploy/add_p_users', _EDIT_FULLL, array("detail_user" => $ret));
    }
    function pedit()
    {
        $id=$this->input->post("id", true);
        $this->permision->check("eemploy_edit", 0, 1);
        $this->load->model('eemploy/eemploy_model');
        $this->system->json_out($this->eemploy_model->add_users($id));
    }
